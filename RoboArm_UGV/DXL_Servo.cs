...
namespace RoboArm_UGV
{
    class DXL_Servo
    {
        ...
        // Default setting
        private Int16 DXL_ID { get; } = 001;                 // Dynamixel ID: 1
        ...
        public int DXL_MINIMUM_POSITION_VALUE { get;} = -1000;                 // Dynamixel will rotate between this value
        public int DXL_MAXIMUM_POSITION_VALUE { get;} = 400;                // and this value (note that the Dynamixel would not move when the position value is out of movable range. Check e-manual about the range of the Dynamixel you use.)
        private const int DXL_MOVING_STATUS_THRESHOLD = 10;                  // Dynamixel moving status threshold
        ...
        public Int16 dxl_present_position { get; } = 0;         // chang to what the starting position will be
        private int port_num = 0;
        private byte dxl_error = 0;
        #endregion Private Properties
        #region Constructors
        // class constructors
        public DXL_Servo(Int16 ID, int Max, int Min)
        {
            DXL_ID = ID;
            DXL_MAXIMUM_POSITION_VALUE = Max;
            DXL_MINIMUM_POSITION_VALUE = Min;
            setup_port();
        ...
        public bool Move_To(Int16 my_goal_posi)
        {
            // Write a check to make sure the goal position is within range and allowable

            // Write goal position
            dynamixel.write2ByteTxRx(port_num, PROTOCOL_VERSION, DXL_ID, ADDR_MX_GOAL_POSITION, my_goal_posi);
            if ((dxl_comm_result = dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION)) != COMM_SUCCESS)
            {
                dynamixel.printTxRxResult(PROTOCOL_VERSION, dxl_comm_result);
                return false;
